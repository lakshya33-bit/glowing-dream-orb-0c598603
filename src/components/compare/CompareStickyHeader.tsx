...
interface StickyHeaderProps { selected: CreditCard[]; onRemove: (id: string) => void; visible: boolean; }
        ...
        <div className={`grid gap-4 ${selected.length <= 2 ? "grid-cols-2 max-w-2xl mx-auto" : selected.length <= 3 ? "grid-cols-3 max-w-4xl mx-auto" : "grid-cols-2 lg:grid-cols-4"}`}>
          {selected.map((card) => (
            <div key={card.id} className="flex items-center gap-2">
              ...
              <button onClick={() => onRemove(card.id)} className="p-1 rounded-md hover:bg-destructive/10 text-muted-foreground hover:text-destructive transition-colors">
                <X className="w-3 h-3" />
              </button>
            </div>
          ))}
