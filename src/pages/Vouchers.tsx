      ...
      <div className="relative p-4 sm:p-6 pb-4">
        ...
        <div className="relative flex items-start justify-between">
          <div className="flex items-center gap-3 sm:gap-4">
            <div className="w-12 h-12 sm:w-16 sm:h-16 rounded-2xl flex items-center justify-center shadow-lg flex-shrink-0" style={{ background: `linear-gradient(135deg, ${voucher.color}25, ${voucher.color}10)`, border: `1px solid ${voucher.color}30` }}>
          ...
          <div className="flex items-center gap-1.5">
            ...
            <Link to={`/vouchers/${voucher.id}`} onClick={onClose} className="p-2 sm:p-2.5 rounded-xl glass-card text-muted-foreground hover:text-foreground transition-colors active:scale-90">
        ...
        <div className="flex items-center justify-between">
          <div className="flex gap-1.5 overflow-x-auto">
            {voucher.denominations.slice(0, 3).map((d) => (
              <span key={d} className="text-[10px] px-2 py-0.5 rounded-lg bg-secondary/40 text-muted-foreground flex-shrink-0">₹{d}</span>
            ))}
            {voucher.denominations.length > 3 && (
              <span className="text-[10px] px-2 py-0.5 rounded-lg bg-secondary/40 text-muted-foreground">+{voucher.denominations.length - 3}</span>
            )}
          </div>
          <Link to={`/vouchers/${voucher.id}`} onClick={onClose} className="text-xs text-gold hover:text-gold-light transition-colors flex items-center gap-1 font-medium flex-shrink-0">
